...
export class UrlController {
  ...
  getSingleUrl = (req: Request, res: Response) => {
    ...
    this.urlModel.getSingleUrl({urlId: parseInt(urlId)}, (err, urls) => {
      ...
      res.status(200).json({
        success: true,
        message: "The URLs retrieved successfully",
        data: urls
      });
    });
};
...
updateUrl = (req: Request, res: Response) => {
  ...
  const urlId = parseInt(req.params.urlId, 10); // Ensure urlId is parsed as an integer
  ...
  this.urlModel.getSingleUrl({ urlId }, (err, urls) => {
    if (err) {
    ...
    const { title, description, newUrl } = updateData;
    this.urlModel.updateUrl({ urlId, title, description, newUrl }, (updateErr, result) => {
      if (updateErr) {
